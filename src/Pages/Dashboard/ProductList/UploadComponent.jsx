...
//   // Get status message based on image count
//   const getStatusMessage = () => {
//     const count = fileList.length;
//     return `${count}/3 - ${count === 0 ? "No" : count} image${
//       count !== 1 ? "s" : ""
//     } uploaded`;
//   };
...
//   return (
//     <div>
//       <div className="mb-2 text-sm font-medium">{getStatusMessage()}</div>
...
//       <div
//         className={`mt-3 flex flex-col gap-2 h-24${
//           fileList ? "h-24" : null
//         } overflow-auto`}
//       >
//         {fileList.map((file) => (
...
const UploadComponent = ({ onFileUpload, existingFiles }) => {
  ...
  const handleChange = ({ fileList }) => {
    ...
    // Send only valid (uploaded) files to the parent
    onFileUpload(
      updatedFiles.filter((file) => file.status === "done" || file.url)
    );
  };
